...
/*
 * This sample program calculates CPU usage for each program name for each time period.
 * 
 ...
 */
public class CpuByProgramName
{
	// Limit the number of programs to display in the report.
	private static final int TOP_LIMIT = 50;

	private static void printUsage()
	{
	    System.out.println("Usage: CpuByProgramName [-month|-day|-hour] <input-name> [<input-name> ...]");
	...
	// Include jobs: If program are specified, only those programs will be included.
	// Uncomment and update as required.
	private static Set<String> includeProgram = 
			Set.of(
					// "ABCDE",
					// "ABCDF",
					// "ABCDG"
	...
	// Exclude jobs: Any programs specified will be excluded.
	// Uncomment and update as required.
	private static Set<String> excludeProgram = 
			Set.of(
					// "ABCDE",
					// "ABCDF",
					// "ABCDG"
            ...
            Grouping grouping)
	{
	    for (SmfRecord record : reader)
	    {
	        ...
	        // See if smf30exn has a value - it tells us the unix program name.
	        // if not available, use the smf30pgm name.
	        String program = r30.identificationSection().smf30exn() == null || r30.identificationSection().smf30exn().length() == 0 ?
	        		r30.identificationSection().smf30pgm()
	        		: r30.identificationSection().smf30exn();
	    	
	    	if (includeRecord(r30, program))
	    	{
        		...
        		CpuStats cpuStats = stats
                        ...
                        .computeIfAbsent(program, p -> new CpuStats());
	...
	private static boolean includeRecord(Smf30Record r30, String program) 
	{
		// program is specifically excluded
		if (excludeProgram.contains(program)) return false;
		
		// the list of program to include is not empty, but does not include this program
		if (!includeProgram.isEmpty() && !includeProgram.contains(program)) return false;
	...
	{
		// write heading
        System.out.format("%-19s %-12s %-20s %14s %14s %14s %14s%n", 
        		"Time", 
        		"System", 
        		"Program", 
        		"CP Time", 
	            ...
	            .forEachOrdered(timeEntry ->
	    {
			...
			systems.entrySet().stream()
	                ...
	                {
	                	List<Map.Entry<String, CpuStats>> sortedPrograms = systemEntry.getValue().entrySet().stream()
	                		.sorted(Comparator
	                	...
	                	int printedCount = Math.min(TOP_LIMIT, sortedPrograms.size());
	                	for (int i = 0; i < printedCount; i++)
	                	{
	                		Map.Entry<String, CpuStats> programEntry = sortedPrograms.get(i);
	                		String program = programEntry.getKey();
	                		CpuStats cs = programEntry.getValue();
	                        System.out.format("%-19s %-12s %-20s %14s %14.2f %14s %14.2f%n", 
	                	...
	                	if (sortedPrograms.size() > TOP_LIMIT)
	                	{
	                		CpuStats other = CpuStats.sum(sortedPrograms.subList(TOP_LIMIT, sortedPrograms.size()).stream()
	        ...
	        System.out.format("%n%-19s %-12s %-20s %14s %14.2f %14s %14.2f%n",
	        		grouping.periodHeading(time),
	        		"",
	        		"Total",
	    ...
	    public void add(Smf30Record r30)
	    {
	    	...
	    	if (processorAccountingSection != null)
	    	{
		    	...
		    	switch (r30.subType())
		    	{
			    	case 2:
			    	case 3:
			    		// take CPU time from subtypes 2 and 3 to capture running jobs/steps 
